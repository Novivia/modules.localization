...
export default class LocaleManager {
  ...
  static getNewMoment(date = new Date()) {
    return LocaleManager.moment(date);
  }
  ...
  formatDate(
    ...
    const momentDate = LocaleManager.getNewMoment(date);
  ...
  formatMoney(
    ...
    // FIXME: Memoize/cache the formatter for a given setting combination.
  ...
  initialize() {
    ...
    return (this.ready = this.initializeInternally());
  }
  ...
  setLocale(locale) {
    return new Promise(
      (resolve, reject) => this.i18next.changeLanguage(
        locale,
