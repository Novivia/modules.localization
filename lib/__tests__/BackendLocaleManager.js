...
describe(
  ...
  () => {
      ...
      () => {
        ...
        it(
          ...
          () => expect(localeManager.getTimezoneData()).toEqual({
            abbreviation: "",
            offset: moment().isDST() ? -4 : -5,
            offsetRaw: moment().isDST() ? -240 : -300,
            offsetString: moment().isDST() ? "-4" : "-5",
            timezone: moment().isDST() ? "-4" : "-5",
          }),
        );
              ...
              () => {
                ...
                it(
                  ...
                  () => {
                    ...
                    )).toBe("January 31, 2017 1:55 AM\xA0IST");
                    expect(localization.formatDate(
                      new Date("2017-01-30T20:25:41.380Z"),
                      {timezone: "Australia/Eucla"},
                    )).toBe("January 31, 2017 5:10 AM\xA0+0845");
                    expect(localization.formatDate(
                      new Date("2017-01-30T20:25:41.380Z"),
                      {
                        format: "LT",
    ...
    describe(
      "when failing to load resources initially",
      () => {
        ...
        it(
          ...
          ).catch(e => expect(e).toEqual([loadingError])),
        );
      },
    );

    describe(
      "when failing to change the locale",
      () => {
        const loadingError = new Error("Loading language resources failed.");
        const localeManager = new BackendLocaleManager({
          i18nextConfiguration: {
            customLoader: (language, namespace, callback) => callback(
              loadingError,
            ),
            languageWhitelist: ["fr"],
            namespaceList: ["general"],
          },
        });
        ...
        it(
          ...
          () => localeManager.setLocale("fr").then(
            () => fail("Changing the locale should have failed."),
          ).catch(
