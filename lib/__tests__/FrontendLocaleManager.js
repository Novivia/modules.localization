...
describe(
  ...
  () => {
    ...
    beforeAll(async () => {
      // Ensure our polyfill gets loaded and works.
      global.Intl = undefined;

      localeManager = new FrontendLocaleManager();

      await localeManager.ensureLocaleIsLoaded({locale: "fr-CA"});

      // Ensure code coverage for the cache hit.
      await localeManager.ensureLocaleIsLoaded({locale: "fr-CA"});
    });
    ...
    describe(
      "when the Intl polyfill is present",
      () => {
        let globalIntl;
        let globalIntlPolyfill;
        let myLocaleManager;

        beforeAll(async () => {
          // Swap polyfills.
          globalIntl = global.Intl;
          globalIntlPolyfill = global.IntlPolyfill;
          global.Intl = {};
          global.IntlPolyfill = {};

          myLocaleManager = new FrontendLocaleManager();
          global.IntlPolyfill = undefined;
        });

        it(
          "should attempt to load polyfill locales",
          () => myLocaleManager.ensureLocaleIsLoaded({locale: "fr-CA"}),
        );

        afterAll(() => {
          // Swap polyfills back.
          global.Intl = globalIntl;
          global.IntlPolyfill = globalIntlPolyfill;
        });
      },
    );

    describe(
      "when the Intl polyfill is not present",
      () => {
        let globalIntl;
        let globalIntlPolyfill;
        let myLocaleManager;
        ...
        it(
          "should not attempt to load polyfill locales",
          () => myLocaleManager.ensureLocaleIsLoaded({locale: "fr-CA"}),
        );
    ...
    describe(
      ...
      () => {
        ...
        it(
          ...
          () => expect(
            FrontendLocaleManager.getNewMoment(),
    ...
    describe(
      ...
      () => {
        ...
        it(
          "should allow to change the locale",
          async () => {
            expect(localeManager.getCurrentLocale()).toBe("en-US");

            await localeManager.setLocale("fr-CA");

            expect(localeManager.getCurrentLocale()).toBe("fr-CA");
          },
        );
      },
    );
    ...
    describe(
      ...
      () => {
        ...
        localeManager.setLocale("fr-CA").then(
            () => console.log("win"),
          ).catch(
            () => console.log("fail"),
          );
